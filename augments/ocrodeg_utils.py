...
class FastPrintlike:
    ...
    def __call__(self, x):
        ...
        xs = x.shape
        try:
            # Ensure dimensions are sufficient for random coordinate generation
            max_x = max(0, self.ink.shape[1] - xs[0])  # Ensure non-negative
            max_y = max(0, self.ink.shape[2] - xs[1])  # Ensure non-negative

            if max_x > 0 and max_y > 0:
                ink_x = np.random.randint(0, max_x)
                ink_y = np.random.randint(0, max_y)
            else:
                # If dimensions are not adequate, return the blurred image
                # print("Image dimensions are too small for applying ink.")
                return blurred  # Could return 'x' or some other default behavior

            ink = self.ink[np.random.randint(0, 10), ink_x: ink_x + xs[0], ink_y: ink_y + xs[1]]
            assert ink.shape == xs, "Ink and image dimensions must match"
        except ValueError as e:
            print(f"An error occurred when trying to apply ink: {e}")
            return blurred  # Return the blurred image as a fallback
