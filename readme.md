...
Run the following once (recommended inside a virtualenv/conda env, but `--user` also works):
  * **WSL/Linux-based OS**: WSL (for windows) or Linux based OS e.g. ubuntu, fedora, arch linux etc. is required.
  * **Miniconda or Anaconda**: Miniconda is recommended for a minimal installation. You can find the [Setup Instruction HERE!](https://www.anaconda.com/docs/getting-started/miniconda/install).
...
## 3) IAM fine-tuning (single command, single gpu)
Set `--base_path` and `--train_data_dir` properly.
```bash
mkdir models/New_F_no_retnorm_8_GPU_Bi_ret_12_synth_increase_gamma_along_layers_RetNet
cp models/no_retnorm_8_GPU_Bi_ret_12_synth_increase_gamma_along_layers_RetNet/ckpt.pt models/New_F_no_retnorm_8_GPU_Bi_ret_12_synth_increase_gamma_along_layers_RetNet
python train.py --run_name=New_F_no_retnorm_8_GPU_Bi_ret_12_synth_increase_gamma_along_layers_RetNet --base_path=/mnt/c/Users/chkim/PycharmProjects/DRetHTR --train_data_dir=dataset/IAM_deslanted --decoder=Bi_ret_RetNet_Sinusoidal_to_out --batch_size=16 --start_epochs=1 --epochs=1000 --lr=5e-5 --weight_decay=1e-03 --lr_scheduler=CosineAnnealingLR --cnn_dropout=0.3 --decoder_dropout=0.3 --img_emb_dropout=0.1 --decoder_emb_dropout=0.1 --p_aug=0.5 --mode=train_inference_recurrent --img_width=2227 --img_height=64 --patch_size=4 --num_channels=1 --embed_dim=768 --d_model=768 --decoder_attention_heads=12 --decoder_ffn_dim=3072 --decoder_depth=12 --cosineRestartEpoch=30 --split=A --weight_init=sd0.02 --bias_init=0 --label_smooth --modeldir=models --feature_extractor=efficientnet_v2_s --gamma_subtracter=0.86 --increase_gamma_along_layers --use_pre_trained_backbone --ret_norm --eval_cycle=1 --load_weight_from_DDP
```
---

## 4) IAM fine-tuning (SLURM)

If you want Multi-GPU training, submit the training job via SLURM:
```bash
...
## 5) IAM test (single command, single gpu)
...
- **Paths:** Update `--train_data_dir` and `--base_path` to match your filesystem if needed
